...
{id:'raspberry',name:'Pernacchia',emoji:'😛',special:'raspberry'},
...
function raspberrySynth(){const c=audioCtx(),t=c.currentTime,d=.85,sr=c.sampleRate,buf=c.createBuffer(1,Math.floor(sr*d),sr),ch=buf.getChannelData(0);for(let i=0;i<ch.length;i++){const x=i/sr;const pulse=(Math.sin(2*Math.PI*27*x)>0?1:.18);const wobble=.55+.45*Math.sin(2*Math.PI*5*x);ch[i]=(Math.random()*2-1)*pulse*wobble*(1-i/ch.length)}const src=c.createBufferSource(),f=c.createBiquadFilter(),g=c.createGain();src.buffer=buf;f.type='bandpass';f.frequency.value=520;f.Q.value=.7;g.gain.setValueAtTime(.26,t);g.gain.exponentialRampToValueAtTime(.001,t+d);src.connect(f).connect(g).connect(c.destination);src.start(t)}
function play(s){if(!enabled)return;stopAudio();if(s.special==='raspberry'){raspberrySynth();return}if(!s.audio){fallbackBeep();return}const a=new Audio(s.audio);a.preload='auto';a.volume=1;currentAudio=a;const p=a.play();if(p&&p.catch)p.catch(()=>fallbackBeep())}
